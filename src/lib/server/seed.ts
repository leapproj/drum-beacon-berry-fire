...
export async function runErpSeed(sql: Sql) {
  ...
  await insert(sql, "license_packages", [
    {
      ...
      features_json: JSON.stringify(["Festival website + CMS", "Event calendar", "Basic participant registration"]),
      commission_pct: 0,
    },
    {
      id: "pkg_command",
      ...
      features_json: JSON.stringify(["Everything in Starter Site", "ePASS + printable credentials", "Gate staff access keys", "Participant portal"]),
      commission_pct: 0,
    },
    {
      id: "pkg_intel",
      ...
      features_json: JSON.stringify(["Everything in Organizer Command", "Turnout & income analytics", "AI Festival Organizer", "Sponsor ROI indicators"]),
      commission_pct: 0,
    },
    {
      id: "pkg_copartner",
      ...
      features_json: JSON.stringify(["TukodPH builds the digital festival", "30% of digital sponsor income only", "Physical sponsors remain yours", "Shared command center"]),
      commission_pct: 30,
    },
  ]);
  ...
  await insert(sql, "festival_licenses", [
    ...
    { id: "lic_d", festival_id: "fst_diyandi2026", package_id: "pkg_copartner", user_id: "op_diyandi", status: "active" },
...
export async function runOperatorSeed(sql: Sql) {
  ...
  for (const a of accounts) {
    const existing = await sql.query<{ id: string }>(
      `select id from operator_accounts where id = $1 or lower(username) = $2`,
      [a.id, a.username],
    );
    if (existing[0]) {
      await sql.query(
        `update operator_accounts set pass_hash = $1, kind = $2, display_name = $3, organization_name = $4, contact_email = $5 where id = $6`,
        [hashPass(a.pass), a.kind, a.display_name, a.organization_name, a.contact_email, existing[0].id],
      );
    } else {
      await sql.query(
