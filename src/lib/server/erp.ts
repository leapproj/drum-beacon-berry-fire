...
async function provisionFestival(sql: Sql, opts: any) {
	...
	if (opts.copartner) await sql.query<any>(`insert into copartner_agreements (id, festival_id, user_id, status, commission_pct, notes)
...
export const payAndPublish = createServerFn({ method: "POST" }).middleware([MW]).validator((input: any) => input).handler(async ({ context, data }) => {
	...
	if (copartner) await sql.query<any>(`insert into copartner_agreements (id, festival_id, user_id, status, commission_pct, notes) values ($1,$2,$3,'requested',30,'TukodPH operates the digital festival.')`, [newId("cpa"), data.festivalId, context.userId]);
