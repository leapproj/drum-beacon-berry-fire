...
export const getSspOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const agreements = await sql.query<any>(`select a.*, a.created_at::text as created_at, f.name as festival_name
    ...
    return {
      ...
      events,
      agreements,
      commission: Math.round((commissionRow[0]?.n ?? 0) * 0.3),
      me,
    };
  });
...
export const getSspIntelligence = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const byFestival = await sql.query<any>(`select f.id, f.name, f.status, f.copartner, f.city, f.slug,
    ...
    const commission = Math.round(
      byFestival.filter((f: any) => f.copartner).reduce((sum: number, f: any) => sum + f.digital * 0.3, 0),
    );
...
export const createFestivalTenant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const packageId = copartner ? "pkg_copartner" : data.package_id || "pkg_command";
    await sql.query<any>(
      ...
      [
        ...
        copartner,
        copartner
          ? "TukodPH co-partner — digital festival operated from Headquarters"
    ...
    if (copartner) {
      await sql.query<any>(
        `insert into copartner_agreements (id, festival_id, user_id, status, commission_pct, notes)
         values ($1,$2,$3,'active',30,'TukodPH HQ created this tenant as digital co-partner. 30% of digital sponsor income.')`,
        [newId("cpa"), id, context.userId],
      );
    }
    await audit(sql, context.userId, "festival.create", "festival", id, { slug, copartner });
    return { id, slug, copartner };
  });
...
export const setFestivalCopartner = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    await sql.query<any>(
      `update festivals set copartner = $1, package_id = case when $1 then 'pkg_copartner' else package_id end where id = $2`,
      [copartner, data.id],
    );
    if (copartner) {
