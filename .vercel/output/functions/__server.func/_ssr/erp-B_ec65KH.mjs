...
import { a as requireFestivalMember, o as requireSsp, r as isSsp, t as audit } from "./helpers-ChFRDPiM.mjs";
import { t as ensureSeed } from "./seed-CmrgG_eq.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/erp-B_ec65KH.js
var PLAN_KEYS = [
	{
		key: "identity",
...
async function provisionFestival(sql, opts) {
	...
	if (opts.copartner) await sql.query(`insert into copartner_agreements (id, festival_id, user_id, status, commission_pct, notes)
       values ($1,$2,$3,'requested',30,'TukodPH operates the digital festival. 30% of digital sponsor income.')`, [
		newId("cpa"),
		id,
		opts.userId
	]);
...
var payAndPublish = createServerFn({ method: "POST" }).middleware([tenantMiddleware]).validator((input) => input).handler(payAndPublish_createServerFn_handler, async ({ context, data }) => {
	...
	if (copartner) await sql.query(`insert into copartner_agreements (id, festival_id, user_id, status, commission_pct, notes) values ($1,$2,$3,'requested',30,'TukodPH operates the digital festival.')`, [
		newId("cpa"),
		data.festivalId,
		context.userId
	]);
