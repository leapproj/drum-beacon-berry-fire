...
//#region node_modules/.nitro/vite/services/ssr/assets/operator-auth-BI2UDnKE.js
...
var signInTenant = createServerFn({ method: "POST" }).validator((input) => input).handler(signInTenant_createServerFn_handler, async ({ data }) => {
	...
	if (!row || row.kind !== "tenant" || !verifyPass(data.password, row.pass_hash)) throw new Error("Invalid ID or passkey");
	return issue(row);
});
...
var signInSsp = createServerFn({ method: "POST" }).validator((input) => input).handler(signInSsp_createServerFn_handler, async ({ data }) => {
	...
	const username = data.username.trim().toLowerCase();
	const row = (await sql.query(`select * from operator_accounts where lower(username) = $1`, [username]))[0];
	if (!row || row.kind !== "ssp" || !verifyPass(data.password, row.pass_hash)) throw new Error("Invalid operator credentials");
	return issue(row);
});
...
//#endregion
export { getOperatorMe_createServerFn_handler, signInSsp_createServerFn_handler, signInTenant_createServerFn_handler, signUpTenant_createServerFn_handler };
