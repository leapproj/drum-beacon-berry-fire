...
import { a as qrPayload } from "./createServerRpc-L9LYBe9K.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/seed-OfWLm1DM.js
async function insert(sql, table, rows) {
	for (const row of rows) {
		const keys = Object.keys(row);
		const placeholders = keys.map((_, i) => `$${i + 1}`).join(",");
		await sql.query(`insert into ${table} (${keys.join(",")}) values (${placeholders}) on conflict do nothing`, keys.map((k) => row[k]));
	}
}
async function runSeed(sql) {
	if ((await sql.query(`select value from app_meta where key = 'seeded'`))[0]?.value === "1") return;
	...
	await insert(sql, "festivals", [
		{
			...
			description: "Cagayan de Oro’s city-wide celebration of kinship, street culture, and Kagay-anon craft. Physical stages, night markets, and a digital watch party run as one tenant.",
			logo_text: "HG",
			city: "Cagayan de Oro",
			province: "Misamis Oriental",
	...
	await insert(sql, "venues", [
		{
			...
			name: "Gaston Park",
			address: "Corrales Ave",
			capacity: 8e3,
			kind: "outdoor",
			notes: "Opening & civic rites"
		},
		{
			id: "ven_xu",
			festival_id: "fst_higalaay2026",
			name: "Xavier University Grounds",
			address: "Corrales Avenue",
			capacity: 5e3,
			kind: "campus",
			notes: "Street dance spillover"
		},
		{
			id: "ven_divisoria",
			festival_id: "fst_higalaay2026",
			name: "Divisoria Night Market",
		...
		},
		{
			id: "ven_cityhall",
			festival_id: "fst_higalaay2026",
			name: "City Hall Grounds",
			address: "Capistrano St",
			capacity: 3e3,
			kind: "civic",
			notes: "Forum & press"
		},
		...
		},
		{
			...
			festival_id: "fst_higalaay2026",
			name: "Digital Stage",
			address: "esaulog.ph/live",
			capacity: 5e4,
			kind: "digital",
			notes: "Hybrid watch party"
		},
		{
			id: "ven_esplanade",
			festival_id: "fst_higalaay2026",
			name: "CDO River Esplanade",
			address: "Yacapin Extension",
			capacity: 4e3,
			kind: "outdoor",
			notes: "Culinary trail"
		},
		{
			id: "ven_maria",
			festival_id: "fst_diyandi2026",
			name: "Maria Cristina Falls View",
			address: "Mendoza Park",
			capacity: 4e3,
			kind: "outdoor",
			notes: ""
		},
		{
			...
			name: "Mambajao Town Plaza",
			address: "Mambajao Poblacion",
			capacity: 3500,
			kind: "outdoor",
			notes: "Harvest parade"
		}
	]);
	...
	const people = [
		{
			...
			festival_id: "fst_higalaay2026",
			user_id: null,
			full_name: "Van Zambrano",
			email: "van@example.ph",
			phone: "+63 917 000 1821",
			city: "Cagayan de Oro",
			age_bracket: "25-34",
			status: "active"
		},
		{
			...
			festival_id: "fst_higalaay2026",
			user_id: null,
			full_name: "Lara Kagay-anon",
			email: "lara@example.ph",
			phone: "",
			city: "Cagayan de Oro",
			age_bracket: "18-24",
			status: "active"
		},
		{
			...
			festival_id: "fst_higalaay2026",
			user_id: null,
			full_name: "Jun Pelaez",
			email: "jun@example.ph",
			phone: "",
			city: "Iligan",
			age_bracket: "35-44",
			status: "registered"
		},
		{
			...
			festival_id: "fst_higalaay2026",
			user_id: null,
			full_name: "Nia Camiguin",
			email: "nia@example.ph",
			phone: "",
			city: "Mambajao",
			age_bracket: "25-34",
			status: "active"
	...
	await insert(sql, "epasses", [
		{
			id: "eps_van",
			festival_id: "fst_higalaay2026",
			participant_id: "par_van",
			credential_id: "ESA-0001821",
			qr_payload: qrPayload("ESA-0001821"),
			nfc_id: null,
			status: "active",
			issued_at: "2026-08-10T08:00:00+08:00",
			expires_at: "2026-08-29T23:59:00+08:00"
		},
		{
			id: "eps_lara",
			festival_id: "fst_higalaay2026",
			participant_id: "par_lara",
			credential_id: "ESA-0001822",
			qr_payload: qrPayload("ESA-0001822"),
			nfc_id: null,
			status: "active",
			issued_at: "2026-08-10T08:00:00+08:00",
			expires_at: "2026-08-29T23:59:00+08:00"
		},
		{
			id: "eps_jun",
			festival_id: "fst_higalaay2026",
			participant_id: "par_jun",
			credential_id: "ESA-0001823",
			qr_payload: qrPayload("ESA-0001823"),
			nfc_id: null,
			status: "active",
			issued_at: "2026-08-11T08:00:00+08:00",
			expires_at: "2026-08-29T23:59:00+08:00"
		},
		{
			id: "eps_nia",
			festival_id: "fst_higalaay2026",
			participant_id: "par_nia",
			credential_id: "ESA-0001824",
			qr_payload: qrPayload("ESA-0001824"),
			nfc_id: null,
			status: "active",
			issued_at: "2026-08-12T08:00:00+08:00",
			expires_at: "2026-08-29T23:59:00+08:00"
		}
	]);
	const regs = [];
	for (const p of people) for (const ev of [
		"evt_kahimunan",
		"evt_msme",
		"evt_watch",
		"evt_culinary",
	...
	await insert(sql, "gates", [
		...
		},
		{
			id: "gate_night",
			festival_id: "fst_higalaay2026",
			event_id: "evt_friendship",
			name: "Night Gate"
		},
		{
			id: "gate_diyandi",
			festival_id: "fst_diyandi2026",
			event_id: "evt_diyandi_open",
			name: "Falls Gate"
		},
		{
			id: "gate_lanzones",
			festival_id: "fst_lanzones2026",
			event_id: "evt_lanzones_parade",
			name: "Plaza Gate"
		}
	]);
	await insert(sql, "gate_access_keys", [
		{
			...
			event_id: "evt_kahimunan",
			gate_id: "gate_a",
			code: "HIGALAAY-GATE-A",
			staff_role: "usher",
			valid_from: "2026-08-22T00:00:00+08:00",
			valid_until: "2026-08-23T02:00:00+08:00",
			max_devices: 12,
			permission_scope: "checkin",
			active: true
		},
		{
			id: "key_open",
			...
			event_id: "evt_opening",
			gate_id: "gate_open",
			code: "HIGALAAY-OPENING",
			staff_role: "usher",
			valid_from: "2026-08-16T00:00:00+08:00",
			valid_until: "2026-08-17T02:00:00+08:00",
			max_devices: 20,
			permission_scope: "checkin",
			active: true
		},
		{
			...
			valid_until: "2026-08-29T02:00:00+08:00",
			max_devices: 8,
			permission_scope: "checkin",
			active: true
		},
		{
			...
			festival_id: "fst_diyandi2026",
			event_id: "evt_diyandi_open",
			gate_id: "gate_diyandi",
			code: "DIYANDI-GATE-A",
			staff_role: "usher",
			valid_from: "2026-09-20T00:00:00+08:00",
			valid_until: "2026-09-30T02:00:00+08:00",
			max_devices: 10,
			permission_scope: "checkin",
			active: true
		},
		{
			...
			festival_id: "fst_lanzones2026",
			event_id: "evt_lanzones_parade",
			gate_id: "gate_lanzones",
			code: "LANZONES-GATE-A",
			staff_role: "usherette",
			valid_from: "2026-10-15T00:00:00+08:00",
			valid_until: "2026-10-21T02:00:00+08:00",
			max_devices: 8,
			permission_scope: "checkin",
			active: true
		}
	...
	await insert(sql, "checkins", [{
		...
		event_id: "evt_opening",
		gate_id: "gate_open",
		access_key_id: "key_open",
		result: "valid",
		reason: "",
		checked_in_at: "2026-08-16T16:12:00+08:00"
	}, {
		id: "chk_lara_open",
		epass_id: "eps_lara",
		event_id: "evt_opening",
		...
		result: "valid",
		reason: "",
		checked_in_at: "2026-08-16T16:18:00+08:00"
	}]);
	await insert(sql, "badges", [{
		id: "bdg_explorer",
		festival_id: "fst_higalaay2026",
		name: "Festival Explorer",
		description: "Complete three festival events",
		icon_key: "compass"
	}, {
		id: "bdg_friend",
		festival_id: "fst_higalaay2026",
		name: "Higala",
		description: "Checked in to opening rites",
		icon_key: "heart"
	}]);
	await insert(sql, "missions", [{
		id: "msn_three",
		festival_id: "fst_higalaay2026",
		title: "Complete 3 festival events",
		description: "Check in to three programme moments.",
		points: 100,
		badge_id: "bdg_explorer",
		...
		description: "Be at Gaston Park for the opening.",
		points: 40,
		badge_id: "bdg_friend",
		condition_type: "checkins",
		condition_value: 1,
	...
	await insert(sql, "rewards", [{
		id: "rwd_coke",
		festival_id: "fst_higalaay2026",
		name: "Coke festival cup",
		description: "Claim at any Coca-Cola booth.",
		points_cost: 80,
		inventory: 400,
		sponsor_id: "spn_coke"
	}]);
	await insert(sql, "sponsor_campaigns", [{
		...
		sponsor_id: "spn_coke",
		festival_id: "fst_higalaay2026",
		name: "Coca-Cola Festival Challenge",
		description: "Visit an event, scan, complete a mission, claim a cup.",
		mission_id: "msn_three",
		status: "live",
		scans: 1280,
	...
	await insert(sql, "vendors", [
		{
			...
			name: "Kagay Kakanin",
			category: "food",
			description: "Sticky rice and festival sweets.",
			location: "Divisoria Stall 12",
			booster: "booster",
			contact: "kakanin@cdo.ph"
		},
		{
			id: "vnd_loom",
			festival_id: "fst_higalaay2026",
		...
		},
		{
			...
			name: "CDO River Brew",
			category: "beverage",
			description: "Local coffee on the esplanade.",
			location: "Esplanade Cart 4",
			booster: "booster",
			contact: "brew@cdo.ph"
		}
	]);
	await insert(sql, "products", [
		{
			id: "prd_puto",
			vendor_id: "vnd_kagay",
			name: "Puto festival box",
			description: "Shareable kakanin.",
			price_php: 180,
			available: true
		},
		{
	...
	await insert(sql, "surveys", [{
		...
		festival_id: "fst_higalaay2026",
		event_id: "evt_opening",
		title: "Opening night pulse",
		status: "closed"
	}, {
		id: "srv_live",
		festival_id: "fst_higalaay2026",
		event_id: "evt_kahimunan",
		title: "Street dance experience",
		status: "open"
	}]);
	await insert(sql, "survey_questions", [{
		id: "q_open_1",
		survey_id: "srv_open",
		prompt: "How was the opening rite?",
		kind: "choice"
	}, {
		id: "q_live_1",
		survey_id: "srv_live",
		prompt: "Which contingent should win People’s Choice?",
		kind: "choice"
	}]);
	await insert(sql, "festival_pages", [
		{
			...
			slug: "home",
			title: "Higalaay 2026",
			body: "The festival of friendship is live in Cagayan de Oro.",
			published: true
		},
		{
			id: "pg_h_about",
			festival_id: "fst_higalaay2026",
			slug: "about",
			title: "About Higalaay",
			body: "Higalaay is Cagayan de Oro’s city-wide celebration of kinship.",
			published: true
		},
		{
			id: "pg_h_guide",
			...
			slug: "guide",
			title: "Visitor guide",
			body: "Get an ePASS, pick events, and explore Divisoria at night.",
			published: true
		},
		{
			id: "pg_d_home",
			...
			slug: "home",
			title: "Diyandi 2026",
			body: "Falls, faith, and fire — Iligan’s festival in setup.",
			published: true
		},
		{
			id: "pg_l_home",
			...
			slug: "home",
			title: "Lanzones 2026",
			body: "Island harvest, island welcome.",
			published: true
		}
	]);
	await insert(sql, "ai_recommendations", [{
		id: "ai_staff",
		festival_id: "fst_higalaay2026",
		title: "Friendship Night has thin gate coverage",
		body: "Assign two more ushers from the standby roster before 28 August.",
		severity: "warn",
		status: "open",
		kind: "ops"
	}, {
		id: "ai_survey",
		festival_id: "fst_higalaay2026",
		title: "Culinary trail has no survey",
		body: "Add a two-question pulse so MSME satisfaction lands in M&E.",
		severity: "info",
		status: "open",
		kind: "readiness"
	}]);
...
async function runErpSeed(sql) {
	if ((await sql.query(`select value from app_meta where key = 'erp_seeded'`))[0]?.value === "1") return;
	await insert(sql, "license_packages", [
		{
			...
			price_php: 45e3,
			billing: "per season",
			description: "Public festival website, CMS, and a published programme. For organizers who need a site first.",
			features_json: JSON.stringify([
				"Festival website + CMS",
				"Event calendar",
		...
		{
			id: "pkg_command",
			slug: "organizer-command",
			name: "Organizer Command",
			kind: "self_serve",
			price_php: 185e3,
			billing: "per season",
			description: "Full command center: ePASS, gate staff, participant portal, and operations desk.",
			features_json: JSON.stringify([
		...
		{
			id: "pkg_intel",
			slug: "festival-intelligence",
			name: "Festival Intelligence",
			kind: "self_serve",
			price_php: 32e4,
			billing: "per season",
			description: "Command plus analytics, M&E, sponsor income, and AI readiness.",
			features_json: JSON.stringify([
				"Everything in Organizer Command",
				"Turnout & income analytics",
				"AI Festival Organizer",
				"Sponsor ROI indicators"
			]),
			commission_pct: 0
		},
		{
			id: "pkg_copartner",
			slug: "digital-copartner",
			name: "Digital Co-partner",
			kind: "copartner",
			price_php: 0,
			billing: "30% of digital sponsor income",
			description: "TukodPH creates and operates the digital festival. 30% commission on digital sponsor income only. Physical sponsors stay with the organizer.",
			features_json: JSON.stringify([
				"TukodPH builds the digital festival",
				"30% of digital sponsor income only",
				"Physical sponsors remain yours",
	...
	await sql.query(`update festivals set package_id = 'pkg_copartner', copartner = true where id = 'fst_diyandi2026'`);
	...
	for (const fid of [
		...
		for (const item of plan) await sql.query(`insert into planning_items (id, festival_id, key, label, done) values ($1,$2,$3,$4,$5) on conflict do nothing`, [
			`pln_${fid}_${item.key}`,
			fid,
			item.key,
			item.label,
			live || item.key === "identity"
		]);
	}
	await insert(sql, "cms_blocks", [{
		...
		festival_id: "fst_higalaay2026",
		kind: "hero",
		heading: "Higalaay 2026",
		body: "The festival of friendship. Now live in Cagayan de Oro.",
		meta_json: "{}",
		sort_order: 0,
		visible: true
	}, {
		...
		kind: "text",
		heading: "This week",
		body: "Street dance, night market, culinary trail, and a digital watch party — one tenant, one ePASS.",
		meta_json: "{}",
		sort_order: 1,
		visible: true
	}]);
	await insert(sql, "sponsor_income", [
		{
			...
			channel: "physical",
			amount_php: 25e5,
			recognized_on: "2026-08-01",
			note: "Booths and pouring rights"
		},
		{
			id: "inc_coke_d",
			festival_id: "fst_higalaay2026",
			...
			channel: "digital",
			amount_php: 68e4,
			recognized_on: "2026-08-08",
			note: "Watch party + missions"
		},
		{
			id: "inc_smart_d",
			festival_id: "fst_higalaay2026",
			...
			channel: "digital",
			amount_php: 42e4,
			recognized_on: "2026-08-10",
			note: "Connectivity + stream"
		},
		{
			id: "inc_diyandi_d",
			festival_id: "fst_diyandi2026",
			...
			channel: "digital",
			amount_php: 31e4,
			recognized_on: "2026-09-01",
			note: "Pre-sale digital inventory"
		}
	]);
	await insert(sql, "copartner_agreements", [{
		...
		status: "active",
		commission_pct: 30,
		notes: "TukodPH operates the digital festival. 30% of digital sponsor income."
	}]);
	await insert(sql, "festival_licenses", [
		{
			id: "lic_h",
			festival_id: "fst_higalaay2026",
			package_id: "pkg_intel",
			user_id: "op_higalaay",
			status: "active"
		},
		{
...
async function runOperatorSeed(sql) {
	await sql.query(`alter table operator_accounts add column if not exists last_seen_at timestamptz`);
	for (const a of [
		{
			id: "op_ssp_van",
			username: "tukodph_van",
			pass: "926324",
			kind: "ssp",
			display_name: "Van Zambrano",
			organization_name: "TukodPH",
			contact_email: "van@tukodph.com"
		},
		{
			id: "op_ssp_lanz",
			username: "tukodph_lanz",
			pass: "123456",
			kind: "ssp",
			display_name: "Lanz",
			organization_name: "TukodPH",
			contact_email: "lanz@tukodph.com"
		},
		{
			id: "op_ssp_marc",
			username: "tukodph_marc",
			pass: "000000",
			kind: "ssp",
			display_name: "Marc",
			organization_name: "TukodPH",
			contact_email: "marc@tukodph.com"
		},
		{
			id: "op_higalaay",
			username: "higalaay",
			pass: "higalaay2026",
			kind: "tenant",
			display_name: "Higalaay Command",
			organization_name: "Cagayan de Oro City",
			contact_email: "higalaay@cdo.gov.ph"
		},
		{
			id: "op_diyandi",
			...
			pass: "diyandi2026",
			kind: "tenant",
			display_name: "Diyandi Command",
			organization_name: "Iligan City",
			contact_email: "diyandi@iligan.gov.ph"
		},
		{
			id: "op_lanzones",
			...
			pass: "lanzones2026",
			kind: "tenant",
			display_name: "Lanzones Command",
			organization_name: "Province of Camiguin",
			contact_email: "lanzones@camiguin.gov.ph"
		}
	]) {
		const existing = await sql.query(`select id from operator_accounts where id = $1 or lower(username) = $2`, [a.id, a.username]);
		if (existing[0]) await sql.query(`update operator_accounts set pass_hash = $1, kind = $2, display_name = $3, organization_name = $4, contact_email = $5 where id = $6`, [
			hashPass(a.pass),
			a.kind,
			a.display_name,
			a.organization_name,
		...
		else await sql.query(`insert into operator_accounts (id, username, pass_hash, kind, display_name, organization_name, contact_email)
	...
	]) await sql.query(`insert into festival_members (festival_id, user_id, role) values ($1,$2,'admin') on conflict do nothing`, [festivalId, userId]);
	const allFestivals = await sql.query(`select id from festivals`);
	const sspOps = await sql.query(`select id from operator_accounts where kind = 'ssp'`);
	for (const f of allFestivals) for (const op of sspOps) await sql.query(`insert into festival_members (festival_id, user_id, role) values ($1,$2,'admin') on conflict do nothing`, [f.id, op.id]);
	await insert(sql, "staff_members", [
		{
			...
			role: "supervisor",
			phone: "+63 917 000 1821",
			email: "van@higalaay.ph",
			status: "active",
			assigned_event_id: "evt_kahimunan",
			notes: "Gate A lead"
		},
		{
			id: "stf_lara",
			...
			email: "lara@higalaay.ph",
			status: "active",
			assigned_event_id: "evt_opening",
		...
		},
		{
			id: "stf_diyandi",
			festival_id: "fst_diyandi2026",
			full_name: "Iligan Gate Team",
			role: "coordinator",
			phone: "+63 63 221 3032",
			email: "gates@iligan.gov.ph",
			status: "active",
			assigned_event_id: "evt_diyandi_open",
			notes: "Falls viewing deck"
		},
		{
			id: "stf_lanzones",
			festival_id: "fst_lanzones2026",
			full_name: "Camiguin Ushers",
			role: "usher",
			phone: "+63 88 387 1095",
			email: "ushers@camiguin.gov.ph",
			status: "active",
			assigned_event_id: "evt_lanzones_parade",
			notes: "Harvest parade"
		}
	]);
}
