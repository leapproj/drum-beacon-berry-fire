...
//#region node_modules/.nitro/vite/services/ssr/assets/ssp-DI1yyQpn.js
var PLAN_KEYS = [
	{
		key: "identity",
...
var getSspOverview = createServerFn({ method: "GET" }).middleware([sspMiddleware]).handler(getSspOverview_createServerFn_handler, async ({ context }) => {
	...
	const agreements = await sql.query(`select a.*, a.created_at::text as created_at, f.name as festival_name
...
var createFestivalTenant = createServerFn({ method: "POST" }).middleware([sspMiddleware]).validator((input) => input).handler(createFestivalTenant_createServerFn_handler, async ({ context, data }) => {
	...
	await sql.query(`insert into festivals (
		...
		packageId,
		copartner,
		copartner ? "TukodPH co-partner — digital festival operated from Headquarters" : "Provisioned from TukodPH Headquarters"
	]);
	...
	if (copartner) await sql.query(`insert into copartner_agreements (id, festival_id, user_id, status, commission_pct, notes)
...
var setFestivalCopartner = createServerFn({ method: "POST" }).middleware([sspMiddleware]).validator((input) => input).handler(setFestivalCopartner_createServerFn_handler, async ({ context, data }) => {
	...
	await sql.query(`update festivals set copartner = $1, package_id = case when $1 then 'pkg_copartner' else package_id end where id = $2`, [copartner, data.id]);
	if (copartner) {
